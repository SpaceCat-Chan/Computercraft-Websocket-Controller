...
class Camera
{
	 ...
	 *
	 * the vector is either the direction or location, depending on the value of
	 * ViewLock
	 */
	...
	glm::dvec3 GetUpVector() const { return m_Up; }
	...
	/**
	 * \brief sets LookAt to Locked::Position, meaning the camera will try to
	 * look at the same position
	 */
	void LockViewPosition();
	/**
	 * \brief set LookAt to Locked::Direction, meaning the camera will try to
	 * look in the same direction
	 */
	void LockViewDirection();
	/**
	 * \brief set LookAt to Locked::PitchYaw, meaning the camera willl try to
	 * look  in the same direction, but with pitch and yaw
	 */
	 ...
	 * \brief will make the camera look in a direction
	 *
	 * will also set LookAt to Locked::PitchYaw, meaning that direction is
	 * locked and rotation is pitch and yaw based instead of vector based
	 *
	 ...
	 */
	void
	LookIn(double Pitch = 0, double Yaw = glm::radians(-90.0), double Roll = 0);
	/**
	 ...
	 * \param RespectRoll if Roll should be acounted for when changing Pitch and
	 * Yaw
	 *
	...
	void RotateAround(
	    double x,
	    double y,
	    std::optional<glm::dvec3> around = std::nullopt,
	    bool reatain_up = true);

	void Rotate(double x, double y, bool retain_up = true) noexcept;

	private:
	std::tuple<glm::dmat4, glm::dmat4> RotateVector(double x, double y, bool position_fix) const noexcept;
};
